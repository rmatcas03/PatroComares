<?php include("includes/a_config.php"); ?>
<!DOCTYPE html>
<html>

<head>
  <?php include("includes/head-tag-contents.php"); ?>
</head>

<body>

  <?php include("includes/navigation.php"); ?>

  <main>

    <section class="page-section" id="noticias">
      <div class="text-center mt-5">
        <h1>Noticias destacadas</h1>
      </div>
      <div class="container-fluid row mt-3">

        <div class="card col-4 border-0">
          <h2 class="text-center">Fútbol base</h2>
          <img class="card-img-top" src="imgTrabajo\fff.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>

        <div class="card col-4 border-0">
          <h2 class="text-center">Natación</h2>
          <img class="card-img-top" src="imgTrabajo\natacionhd.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>

        <div class="card col-4 border-0">
          <h2 class="text-center">Pádel</h2>
          <img class="card-img-top" src="imgTrabajo\padel.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>
      </div>

      <div class="container-fluid row my-5">

        <div class="card col-4 border-0">
          <h2 class="text-center">Defensa personal</h2>
          <img class="card-img-top" src="imgTrabajo\defensa personalhd.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>

        <div class="card col-4 border-0">
          <h2 class="text-center">Atletismo</h2>
          <img class="card-img-top" src="imgTrabajo\atletismo01.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>

        <div class="card col-4 border-0">
          <h2 class="text-center">Tenis</h2>
          <img class="card-img-top" src="imgTrabajo\tenis.jpg" alt="Card image" style="width:100%">
          <div class="card-body">
            <h4 class="card-title">Lucena fc</h4>
            <p class="card-text">Comienzan las clases de fútbol en nuestra academia municipal</p>
            <a href="#" class="btn btn-secondary">Más información</a>
          </div>
        </div>
      </div>
    </section>

  </main>
  <?php include("includes/footer.php"); ?>
</body>

</html>